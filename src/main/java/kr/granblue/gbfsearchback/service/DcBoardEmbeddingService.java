...
public class DcBoardEmbeddingService {
    ...
    protected void loadAndExecute(Params params, Function<Params, Page<DcBoard>> getNotEmbeddedBoardFunction) {
        // 비동기 작업 리스트
        List<CompletableFuture<List<DcBoardEmbedding>>> futures = new ArrayList<>();
        // 시작시간
        ...
        List<List<DcBoardEmbedding>> embeddedResults = futures.stream().map(CompletableFuture::join).collect(Collectors.toList());
     ...
     */
    @Async
    protected CompletableFuture<List<DcBoardEmbedding>> asyncEmbedAndSave(List<DcBoard> dcBoards) {
        return CompletableFuture.supplyAsync(() -> {
            List<DcBoardEmbedding> boardEmbeddings = new ArrayList<>();
            int reduceLength = 0; // token 초과시 prompt 자를 길이.
            for (int i = 0; i < dcBoards.size(); i++) {
                DcBoard dcBoard = dcBoards.get(i);
                log.info("\n----------------------------------------------------------\n" +
