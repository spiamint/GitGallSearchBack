package kr.granblue.dcscraper.scraper.service.impl;
...
public class DefaultDcScraper implements DcScraper {
    ...
    public void setWebDriverWait(WebDriverWait explicitWait) {
        if (explicitWait == null) {
            this.globalExplicitWait = new WebDriverWait(webDriver, Duration.ofMillis(2000));
        } else {
    ...
    /**
     * 스크래핑 한번하고 드라이버를 끄기 때문에 재시작을 위해 드라이버를 다시 시작
     */
    ...
    protected ScrapeResult scrape(long startPage, long endPage, ScrapingOption scrapingOption) throws InterruptedException {
        ...
        while (pageNum <= maxPageNum) {
            ...
            if (trElements.size() < listNum && trElementsReloadCounter < 5) {
...
//                cutCounter++;
    ...
    protected Elements openListPageAndParse(String executeUrl, int trElementsReloadCounter) {
        ...
        try {
            Thread.sleep(1000); // 리스트 페이지는 좀 더 기다림
    ...
    protected Element openViewPageAndParse(String executeUrl) {
        ...
        try {
            Thread.sleep(500); // boardInterval.  여기에 없으면 차단당하거나 로드 느려짐 (로드 후 기다리기X)
