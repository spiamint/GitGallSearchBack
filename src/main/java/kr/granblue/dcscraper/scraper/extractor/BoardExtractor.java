package kr.granblue.dcscraper.scraper.extractor;

import kr.granblue.dcscraper.scraper.dto.DcBoard;
import kr.granblue.dcscraper.scraper.util.ContentCleaner;
...
//            log.warn("[WARN] gallNum = {} is not number", gallNum);
    ...
    public DcBoard extractFromListPage(Element trElement) {
        String gallNumString = selectBy(trElement, gallNumSelector, gallNumAttr);
        ...
        // 내용 추출
        String title = selectBy(trElement, titleSelector, titleAttr);
        String writer = selectBy(trElement, writerSelector, writerAttr);
        String regDate = selectBy(trElement, regDateSelector, regDateAttr);
        String viewCnt = selectBy(trElement, viewCntSelector, viewCntAttr);
        String recommendCnt = selectBy(trElement, recommendCntSelector, recommendCntAttr);
        String commentCnt = selectBy(trElement, commentCntSelector, null);
        String recommended = selectBy(trElement, recommendSelector, null); // 글 아이콘 이미지에 .icon_recomimg 가 있으면 개념글
    ...
    public String extractContentFromViewPage(Element mainElement) {
        // 내용 추출
        String rawContent = selectBy(mainElement, contentSelector, contentAttr);
...
//        );
    ...
    protected String selectBy(Element trElement, String selector, String attr) {
        try {
            if (selector.equals(commentCntSelector)) { // commentCnt 는 별도추출
                return extractCommentCnt(trElement).toString();
            }
            if (selector.equals(recommendSelector)) { // recommend 는 selector 로 empty 면 false
    ...
    protected Long extractCommentCnt(Element trElement) {
        long result;
        Element commentCntElement = trElement.select(commentCntSelector).first();
