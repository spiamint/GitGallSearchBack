package kr.github.gitgallsearchback.scraper.extractor;
...
public class CommentExtractor {
    ...
    public List<DcComment> extractCommentAndReply(long dcBoardNum, Element liElement, Element liElementPrev) {
        ...
        if (!isReply(liElement)) { // 답글 리스트 여부 확인 (댓글 or 답글list 로 분기)
            ...
            // 답글 리스트
            Elements liElementsReply = liElement.select(replyListItemSelector);
            // 답글 리스트 루프
            for (Element liElementReply : liElementsReply) {
                if (isDeleted(liElementReply)) { // 삭제여부 확인
                    ...
                    // 답글
                    long targetId;
                    if (liElementPrev == null || liElementPrev.select(commentNumSelector).isEmpty()) {
                        // 이전 댓글이 삭제되었거나, 댓글이 아닌 경우 targetNum 을 -1 로 설정
                        targetId = -1;
                    } else {
                        targetId = Long.parseLong(selectBy(liElementPrev, commentNumSelector, commentNumAttr));
                    }
...
//            log.info("CommentExtractor result: {}", result);
    ...
    protected boolean isReply(Element liElement) {
        return presenceSelectBy(liElement, isReplySelector);
    }

    protected boolean isDeleted(Element liElement) {
        return presenceSelectBy(liElement, isDeletedSelector);
    }

    protected boolean isDory(Element liElement) {
        return presenceSelectBy(liElement, isDorySelector);
    }

    protected DcComment extractComment(long dcBoardNum, Element listItem) {
        String commentNum = selectBy(listItem, commentNumSelector, commentNumAttr);
        String writer = selectBy(listItem, commentWriterSelector, commentWriterAttr);
        String content = selectBy(listItem, commentContentSelector, commentContentAttr);
        String regDate = selectBy(listItem, commentRegDateSelector, commentRegDateAttr);
    ...
    public DcComment extractReply(long dcBoardNum, Element listItem, long targetNum) {
        String replyNum = selectBy(listItem, replyNumSelector, replyNumAttr);
        String writer = selectBy(listItem, commentWriterSelector, commentWriterAttr);
        String content = selectBy(listItem, commentContentSelector, commentContentAttr);
        String regDate = selectBy(listItem, commentRegDateSelector, commentRegDateAttr);
        LocalDateTime parsedTime = parseTime(regDate);
    ...
    public DcComment extractDeletedComment(long dcBoardNum, Element liElement) {
        String commentNum = selectBy(liElement, commentNumSelector, commentNumAttr);
        String content = selectBy(liElement, isDeletedSelector, isDeletedAttr);
    ...
    public DcComment extractDeletedReply(long dcBoardNum, Element liElement) {
        String commentNum = selectBy(liElement, replyNumSelector, replyNumAttr);
        String content = selectBy(liElement, isDeletedSelector, isDeletedAttr);
